...
import { dialog_open, TextInput, PassInput, CheckBoxes, TeardownMessage } from "./dialog.jsx";
...
export function mounting_dialog(client, block, mode, forced_options) {
    ...
    const stop_mode_action = {
        mount: _("Stop and mount"),
        unmount: _("Stop and unmount"),
        update: _("Stop and apply")
    };
    ...
    const dlg = dialog_open({
        ...
        Fields: fields,
        Action: {
            Title: mount_point_users.length > 0 ? stop_mode_action[mode] : mode_action[mode],
            DangerButton: mount_point_users.length > 0,
    ...
    dlg.run(_("Checking related processes"),
            utils.add_mount_point_processes(client, usage, old_dir)
                    .then(users => {
                        console.log("USERS", users);
                        mount_point_users = users;
                        dlg.set_attribute("Teardown", TeardownMessage(usage));
                        dlg.set_action_attribute("Title",
                                                 users.length > 0 ? stop_mode_action[mode] : mode_action[mode]);
                        dlg.set_action_attribute("DangerButton", users.length > 0);
                    })
                    .finally(() => {
                        if (block.IdUsage == "crypto" && mode == "mount")
