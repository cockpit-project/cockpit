 ...
 *
 * Converts the given data into a hexadecimal string.  Uses big endian
 * (ie: first output byte will be the high order nibble of the first
 * input byte).
 *
 * This function never fails.  Memory allocation errors will abort the
 * program.
 *
 * The return value of this function needs to be free()d.
 */
static char *
to_hex (const unsigned char *data,
        size_t               size)
{
  size_t length = size * 2 + 1;
  char *output = mallocx (length);

  int str_offset = 0;
  for (int i = 0; i < size; i++)
    {
      int s = snprintf (output + str_offset, length - str_offset, "%02x", data[i]);
      assert (s == 2);
      str_offset += s;
    }

  output[str_offset++] = '\0';
  assert (str_offset == length);
 ...
 *
 * @peer_certificate must be non-%NULL.
 *
...
client_certificate_get_wsinstance (const gnutls_datum_t *certificate)
{
  ...
  return to_hex (digest_data, digest_size);
}
...
client_certificate_random_filename (void)
{
  ...
  return to_hex (random_data, sizeof random_data);
}
...
client_certificate_create_tmpfile (int  dirfd,
                                   int *out_fd)
{
  /* create with no permissions: only root will read this */
  int fd = openat (dirfd, ".", O_TMPFILE | O_WRONLY, 0);
  if (fd == -1)
    {
