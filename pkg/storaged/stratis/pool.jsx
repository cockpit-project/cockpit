...
function get_key_descriptions(pool) {
    ...
    if (client.stratis_interface_revision < 8) {
        ...
        /* The KeyDescriptions property itself has type "v". For a V1
           ...
           Since the property itself has type "v", we need unwrap this
           variant explicitly.

           HACK - https://github.com/stratis-storage/stratisd/issues/3895
           However, change notifications drop the variant
           wrapping for some reason, so we only do the unwrapping when
           "key_descs" is indeed a variant.
         */
        if ("t" in key_descs)
...
function get_clevis_infos(pool) {
    ...
    if (client.stratis_interface_revision < 8) {
        ...
        /* The ClevisInfos property itself has type "v". For a V1 pool
           ...
           HACK - https://github.com/stratis-storage/stratisd/issues/3895
           However, change notifications drop the variant wrapping for
           some reason, so we only do the unwrapping when "clevis_infos" is
           indeed a variant.
         */
...
const StratisV2TokenTable = ({
    ...
    const Slot = ({ test_location, slot, description, actions }) => (
        <Tr data-test-row-location={test_location}>
