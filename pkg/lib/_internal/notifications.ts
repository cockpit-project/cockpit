...
export interface NotificationControlMessage {
    // legacy pre-board form: null clears the entry
...
export class NotificationBoard<T extends Notification> extends EventTarget {
    ...
    // Post (overwriting) this page's entry on the board; equal repeats are
    // deduped.  EXTRA fields ride along in the control message.
    publish(notification: T, extra?: JsonObject): void {
    ...
    clear(extra?: JsonObject): void {
        if (this.#last_sent === null)
            return;
        this.#last_sent = null;
        cockpit.transport.control("notify", { ...extra, board: this.name, notification: null });
    }
