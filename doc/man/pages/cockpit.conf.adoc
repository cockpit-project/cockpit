...
`$XDG++_++CONFIG++_++DIRS` is set, then the first path containing a
...
`UrlRoot`::
  ...
  as nginx. `/cockpit/` and `/cockpit{plus}` are reserved and should not
  be used. For example `/cockpit-new/` is ok. `/cockpit/` and
  `/cockpit{plus}new/` are not.
...
`URL`::
  ...
  it needs to obtain an oauth token. Cockpit will add a redirect++_++uri
