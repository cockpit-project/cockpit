...
function start_pool(uuid, show_devs) {
    ...
    function start_passphrase(passphrase) {
        if (v2_encrypted)
        ...
        dialog_open({
            ...
            Action: {
                ...
                action: function(vals) {
                    return start_passphrase(vals.passphrase);
                }
            }
        });
    }
    ...
    if (!key_desc && !clevis_info && !v2_encrypted) {
        // Not an encrypted pool, just start it
        return start();
    } else if ((key_desc && clevis_info) || v2_encrypted) {
        return start_clevis().catch(unlock_with_keyring);
