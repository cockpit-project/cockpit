...
class WebdriverBidi:
    ...
    def __init__(self, headless: bool = False) -> None:
        ...
        self.homedir = tempfile.TemporaryDirectory(prefix="browser-home-")
        self.download_dir = Path(self.homedir.name) / 'Downloads'
    ...
    async def bidi(self, method, *, quiet: bool = False, timeout: int = 10, **params) -> dict[str, Any]:
...
class ChromiumBidi(WebdriverBidi):
    ...
    async def start_bidi_session(self) -> None:
        ...
        driver = await asyncio.create_subprocess_exec(
                ...
                env=dict(os.environ, HOME=self.homedir.name))
