...
import { InstallProgressCB, MissingPackages, PackageManager, ProgressCB, ResolveError } from './packagemanager-abstract';
...
type object_type = "Package" | "Group" | "Environment" | "Module";
...
export class Dnf5DaemonManager implements PackageManager {
    ...
    async check_missing_packages(pkgnames: string[], progress_cb?: ProgressCB): Promise<MissingPackages> {
        ...
        async function refresh(session: string) {
            ...
            if (resolve_result !== 0)
                throw new ResolveError(`Resolving read_all_repos failed with result={resolve_result}`);
        ...
        async function simulate(session: string) {
            ...
            if (result !== 0)
                throw new ResolveError(`Resolving install failed with result={result}`);

            for (const transaction_item of transaction_items) {
                const [sort, _action, reason, _leftover, pkg] = transaction_item;
    ...
    async install_missing_packages(data: MissingPackages, progress_cb?: InstallProgressCB): Promise<void> {
        ...
        try {
            ...
            if (resolve_result !== 0)
                throw new ResolveError(`Resolving install failed with result={resolve_result}`);
            await call(session, "org.rpm.dnf.v0.Goal", "do_transaction", [{}]);
        } catch (err) {
