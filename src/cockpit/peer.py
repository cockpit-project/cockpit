...
class Peer(CockpitProtocol, SubprocessProtocol, Endpoint):
    ...
    init_future: 'asyncio.Future[JsonObject] | None'
    ...
    async def spawn(self, argv: Sequence[str], env: Sequence[str], **kwargs: Any) -> asyncio.Transport:
        # Not actually async...
        loop = asyncio.get_running_loop()
        user_env = dict(e.split('=', 1) for e in env)
        return SubprocessTransport(loop, self, argv, env=dict(os.environ, **user_env), **kwargs)

    async def start(self, init_host: 'str | None' = None, **kwargs: JsonValue) -> JsonObject:
        ...
        def _connect_task_done(task: 'asyncio.Task[None]') -> None:
    ...
    def start_in_background(self, init_host: 'str | None' = None, **kwargs: JsonValue) -> None:
        def _start_task_done(task: 'asyncio.Task[JsonObject]') -> None:
...
class PeersRoutingRule(RoutingRule):
    ...
    def apply_rule(self, options: JsonObject) -> 'Endpoint | None':
