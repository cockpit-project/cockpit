...
class StorageHelpers:
    ...
    def dialog_val(self, field):
        ...
        if ftype == "text-input-checked":
            if self.browser.is_present(sel + "[type=checkbox]:not(:checked)"):
                return False
            else:
                return self.browser.val(sel + "[type=text]")
        elif ftype == "select":
    ...
    def dialog_set_val(self, field, val):
        ...
        if ftype == "checkbox":
            ...
            if not val:
                self.browser.set_checked(sel + "[type=checkbox]", False)
            else:
                self.browser.set_checked(sel + "[type=checkbox]", True)
