...
const DropdownButtons = ({ buttons }) => {
    if (buttons.length > 1) { // do not display caret for single option
        ...
        return (<div className='btn-group dropdown-buttons-container' key={`dropdown-${caretId}`}>
            <button className='pf-c-button pf-m-secondary' id={buttons[0].id} onClick={mouseClick(buttons[0].action)}>
                {buttons[0].title}
            </button>
    ...
    return (<div className='btn-group'>
        <button className='pf-c-button pf-m-secondary' onClick={mouseClick(buttons[0].action)} id={buttons[0].id}>
            {buttons[0].title}
        </button>
    </div>);
};
