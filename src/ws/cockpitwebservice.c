...
static gboolean on_session_idle_timeout (gpointer user_data);
...
cockpit_web_service_finalize (GObject *object)
{
  ...
  if (self->session_idle_timeout)
    g_source_remove (self->session_idle_timeout);
...
static void
set_session_idle_timeout (CockpitWebService *self, gint64 seconds)
{
  if (self->session_idle_timeout)
    {
      g_source_remove (self->session_idle_timeout);
      self->session_idle_timeout = 0;
    }
  if (seconds > 0)
    {
      g_warning("SET TIMER %ld\n", seconds);
      self->session_idle_timeout = g_timeout_add_seconds (seconds, on_session_idle_timeout, self);
    }
}
  ...
  if (self->callers == 0)
    {
      ...
      set_session_idle_timeout (self, 0);
    }
  g_object_unref (self);
}
...
static void
process_set_idle_timeout (CockpitWebService *self,
                          JsonObject *options)
{
  gint64 seconds;
  if (!cockpit_json_get_int (options, "seconds", 0, &seconds))
    seconds = 0;
  set_session_idle_timeout (self, seconds);
}
                          ...
                          GBytes *payload)
{
  ...
  else if (!channel && g_strcmp0 (command, "set-idle-timeout") == 0)
    {
      process_set_idle_timeout (self, options);
    }
  else if (channel)
    {
...
static gboolean
on_session_idle_timeout (gpointer user_data)
{
  CockpitWebService *self = user_data;
