...
*Origins*::
  ...
  with backslashes (e.g. `+\[::1\]+`) as they are matched using `+fnmatch()+`.
   +
[source,ini]
----
[WebService]
...
*MaxConcurrentConnections*::
  Limits the number of concurrent HTTP connections on the Cockpit port.
  Connection attempts that exceed this limit are rejected early on
  without reading a single byte. This can be used during brute-force
  attacks.  It differs from *MaxStartups*, which protects against too
  many concurrent authentication attempts. Defaults to 500.
*AllowUnencrypted*::
...
*CustomLoginPage*::
  Load the login page from an alternative directory. The directory must
  contain a `+login.html+` and optionally a `+po.js+` file for translations.
  When not set, the hardcoded `+$prefix/share/cockpit/static+` directory is used.
