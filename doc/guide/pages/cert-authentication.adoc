...
`/etc/sssd/pki/sssd++_++auth++_++ca++_++db.pem` or setting the
https://www.mankier.com/5/sssd.conf#Services_Sections-PAM_configuration_options[`pam++_++cert++_++db++_++path`]
...
https://linux.die.net/man/5/sshd_config[/etc/ssh/sshd++_++config].
* For sudo, enable `pam++_++sss++_++gss` as described in the
