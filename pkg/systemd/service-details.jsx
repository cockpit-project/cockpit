...
export class ServiceDetails extends React.Component {
    ...
    componentDidMount() {
        if (this.props.unit.FragmentPath && this.props.unit.FragmentPath.startsWith("/etc/systemd")) {
    ...
    editTimerAction() {
        async function getCommand(bus, serviceUnitName) {
            ...
            // ensure there is exactly one ExecStart= in the unit file that matches "%s -c cmd"
            if (exec.length === 1 && exec[0].length > 1 && exec[0][1].length > 2 && exec[0][1][1] == "-c") {
                return exec[0][1][2];
            } else {
