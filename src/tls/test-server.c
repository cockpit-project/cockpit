...
setup (TestCase *tc, gconstpointer data)
{
  ...
  if (fixture && fixture->certfile)
    {
      /* Set up certs.d directory with 0.crt and 0.key */
      ...
      /* Set up secondary certificate if present */
      if (fixture->certfile2)
        {
          g_autofree gchar *cert_link2 = g_build_filename (certs_dir, "1.crt", NULL);
          g_autofree gchar *key_link2 = g_build_filename (certs_dir, "1.key", NULL);
          g_assert_cmpint (symlink (fixture->certfile2, cert_link2), ==, 0);
          g_assert_cmpint (symlink (fixture->keyfile2, key_link2), ==, 0);
        }
  ...
  if (g_file_test (certs_dir, G_FILE_TEST_IS_DIR))
    {
      ...
      g_autofree gchar *cert_file2 = g_build_filename (certs_dir, "1.crt", NULL);
      g_autofree gchar *key_file2 = g_build_filename (certs_dir, "1.key", NULL);
      g_unlink (cert_file);
      g_unlink (key_file);
      g_unlink (cert_file2);
      g_unlink (key_file2);
