...
function RenameGroupDialogBody({ state, change }) {
    ...
    return (
        <Form isHorizontal onSubmit={apply_modal_dialog}>
            ...
            <HelperText>
                <HelperTextItem variant="warning">{_("There might be permission-based side effects associated with group renaming")}</HelperTextItem>
            </HelperText>
        </Form>
    );
}
            ...
            actions: [
                {
                    ...
                    style: "primary",
                    clicked: () => {
                        return cockpit.spawn(["groupmod", group, "--new-name", state.name], { superuser: "require", err: "message" })
                                .then(() => {
