...
import {
    btrfs_usage, validate_subvolume_name, parse_subvol_from_options, is_multi_device_btrfs_volume
} from "./utils.jsx";
...
async function set_mount_options(subvol, block, vals) {
    ...
    if (!is_multi_device_btrfs_volume(block))
...
function make_btrfs_subvolume_page(parent, backing_block, volume, subvol, path_prefix, subvols) {
    ...
    const card = new_card({
        ...
        props: { subvol, subvols, mount_point, mismount_warning, backing_block, content_block, fstab_config, forced_options },
        actions,
    });
...
const BtrfsSubvolumeCard = ({ card, subvol, subvols, mismount_warning, content_block, backing_block, fstab_config, forced_options }) => {
    return (
