...
`ARRAY of DICT++_++ENTRY with STRING keys 'a++{++s?}'`::
  A javascript plain object with the keys as property names.
`ARRAY of DICT++_++ENTRY with other keys 'a++{++??}'`::
...
`interface` is specified then the DBus bus name of the client is used.
If no `path++_++namespace` is provided then `"/"` will be used.

Proxies will be automatically created for instances of the `interface`
available at the DBus service. The optional `path++_++namespace`
argument can be used to restrict the proxies for instances that have
DBus paths which have the namespace path prefix.
...
`path`::
  ...
  `path++_++namespace` property. It should be a valid DBus object path,
...
DBus `path++_++namespace`. If `interface` is specified only properties
on that DBus interface will be watched.
...
`path`::
  ...
  together with the `path++_++namespace` property.
`path++_++namespace`::
