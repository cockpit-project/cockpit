...
function sudo_polish(msg: string | null) {
    if (!msg)
        return msg;
...
interface SuperuserProxy extends cockpit.DBusProxy {
    Current: string;
    Bridges: unknown[];
...
const UnlockDialog = ({ proxy, host } : { proxy: SuperuserProxy, host: string | undefined }) => {
    const D = useDialogs();
...
const LockDialog = ({ proxy, host } : { proxy: SuperuserProxy, host: string | undefined }) => {
    const D = useDialogs();
    const [error, setError] = useState<string | null>(null);
...
export const SuperuserIndicator = ({ proxy, host } : { proxy: cockpit.DBusProxy | null, host?: string }) => {
    ...
    return <SuperuserDialogs superuser_proxy={proxy as SuperuserProxy}
                             host={host}
...
export const SuperuserButton = () => {
    ...
    return <SuperuserDialogs
               superuser_proxy={proxy as SuperuserProxy}
               create_trigger={create_trigger}
               host={undefined} />;
};
