...
conditions::
  ...
  supported predicates are `+path-exists+` and `+path-not-exists+`.
  Unknown predicates are ignored. This is preferable to using
  `+priority+`, but only available since Cockpit 286.
requires::
