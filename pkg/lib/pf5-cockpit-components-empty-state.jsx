...
import { EmptyState, EmptyStateActions, EmptyStateBody, EmptyStateFooter, EmptyStateHeader, EmptyStateIcon, EmptyStateVariant } from "@patternfly/react-core/dist/esm/components/EmptyState/index.js";
...
export const EmptyStatePanel = ({ title, paragraph, loading, icon, action, isActionInProgress, onAction, secondary, headingLevel, titleSize }) => {
    ...
    let iconEmptyState = null;
    if (loading)
        iconEmptyState = <Spinner className="ct-spinner" size="xl" />;
    if (icon)
        iconEmptyState = <EmptyStateIcon icon={icon} />;

    return (
        <EmptyState variant={EmptyStateVariant.full}>
            <EmptyStateHeader titleText={title} headingLevel={headingLevel}
                icon={iconEmptyState} />
            <EmptyStateBody>
                {paragraph}
            </EmptyStateBody>
