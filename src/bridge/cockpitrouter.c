      ...
      if (match->node)
        {
          ...
          g_message ("  %s: %s", match->name, text);
          g_free (text);
        }
      else if (match->glob)
        {
          g_message ("  %s: glob", match->name);
        }
      else
        {
          g_message ("  %s", match->name);
        }
    }
}
#endif
                            ...
                            gpointer user_data)
{
  ...
  if (!cockpit_json_get_string (options, "payload", NULL, &payload))
    ...
    g_message ("%s: bridge doesn't support 'payload' of type: %s", channel, payload);
              ...
              GBytes *data)
{
  ...
  /* Now go throgh the rules */
    ...
    {
      for (l = self->rules ? self->rules->tail : NULL; l != NULL; l = g_list_previous (l))
        {
...
cockpit_router_dispose (GObject *object)
{
  ...
  for (l = self->rules->tail; l != NULL; l = g_list_previous (l))
    {
      rule = l->data;
      if (rule->destroy)
                    ...
                    GList *bridges)
{
  ...
  if (payload_types)
    cockpit_router_add_payloads (router, payload_types);

  for (l = bridges; l != NULL; l = g_list_next (l))
    cockpit_router_add_bridge (router, l->data);

#ifdef WITH_DEBUG
  for (l = router->rules->tail; l != NULL; l = g_list_previous (l))
...
void
cockpit_router_add_payloads (CockpitRouter *self,
                             CockpitPayloadType *types)
{
  RouterRule *rule;
  guint i;
  ...
  for (i = 0; types[i].name != NULL; i++)
    {
      ...
      rule->user_data = types[i].function;
      rule->matches = g_new0 (RouterMatch, 2);
      rule->matches[0].name = g_strdup ("payload");
      rule->matches[0].glob = g_pattern_spec_new (types[i].name);
      rule->matches[0].node = json_node_init_string (json_node_new (JSON_NODE_VALUE), types[i].name);
      g_queue_push_tail (self->rules, rule);
    }
}
                           ...
                           JsonObject *config)
{
  ...
  /* We start to own the match for our rule */
  json_object_ref (match);
  json_object_remove_member (config, "match");
