...
export function ServerTime() {
    ...
    self.update = function update() {
        return cockpit.exec("date", [], ["+%s:%z"], { err: "message" })
                .then(data => {
    ...
    self.change_time = (datestr, timestr) => cockpit.exec("date", [`--date=${datestr} ${timestr}`], ["+%s"])
            .then(data => {
    ...
    async function set_custom_ntp_timesyncd(config) {
        ...
        await cockpit.exec("mkdir", ["-p", "-m755"], ["/etc/systemd/timesyncd.conf.d"], { superuser: "require" });
        await custom_ntp_config_file.replace(text);
        await cockpit.exec("chmod", ["644"], [conf_path], { superuser: "require" });
    }
    ...
    function set_custom_ntp_chronyd(config) {
        ...
        return cockpit.exec("mkdir", ["-p"], [chronyd_sourcedir], { superuser: "require" })
                .then(() => {
                    if (config.enabled)
    ...
    self.get_timezones = function() {
        return cockpit.exec("/usr/bin/timedatectl", ["list-timezones"])
