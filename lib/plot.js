 ...
 *
 *
 * - plot = module.plot(element, x_range, [x_stop])
 *
...
module.plot = function plot(element, x_range_seconds, x_stop_seconds) {
    var options = { };
    var result = { };
...
module.plot_simple_template = function simple() {
    return {
        colors: plot_colors,
        legend: { show: false },
        series: { shadowSize: 0,
        ...
        xaxis: { tickLength: 0,
                 mode: "time",
                 tickFormatter: module.format_date_tick,
...
module.memory_ticks = function memory_ticks(opts) {
...
module.format_date_tick = function format_date_tick(val, axis) {
    function pad(n) {
        var str = n.toFixed();
        if(str.length == 1)
            str = '0' + str;
...
module.bytes_tick_unit = function bytes_tick_unit(axis) {
    return cockpit.format_bytes(axis.max, 1024, true)[1];
};

module.format_bytes_tick_no_unit = function format_bytes_tick_no_unit(val, axis) {
    return cockpit.format_bytes(val, module.bytes_tick_unit(axis), true)[0];
};

module.format_bytes_tick = function format_bytes_tick(val, axis) {
    return cockpit.format_bytes(val, 1024);
};

module.bytes_per_sec_tick_unit = function bytes_per_sec_tick_unit(axis) {
    return cockpit.format_bytes_per_sec(axis.max, 1024, true)[1];
};

module.format_bytes_per_sec_tick_no_unit = function format_bytes_per_sec_tick_no_unit(val, axis) {
    return cockpit.format_bytes_per_sec(val, module.bytes_per_sec_tick_unit(axis), true)[0];
};

module.format_bytes_per_sec_tick = function format_bytes_per_sec_tick(val, axis) {
    return cockpit.format_bytes_per_sec(val, 1024);
};

module.bits_per_sec_tick_unit = function bits_per_sec_tick_unit(axis) {
    return cockpit.format_bits_per_sec(axis.max*8, 1000, true)[1];
};

module.format_bits_per_sec_tick_no_unit = function format_bits_per_sec_tick_no_tick(val, axis) {
    return cockpit.format_bits_per_sec(val*8, module.bits_per_sec_tick_unit(axis), true)[0];
};

module.format_bits_per_sec_tick = function format_bits_per_sec_tick(val, axis) {
    return cockpit.format_bits_per_sec(val*8, 1000);
};

module.setup_plot_controls = function setup_plot_controls(container, element, plots) {
...
module.setup_complicated_plot = function setup_complicated_plot(graph_id, grid, series, options) {
