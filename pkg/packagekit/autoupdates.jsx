...
class AptImpl extends ImplBase {
    ...
    async setConfig(enabled, type, day, time) {
        ...
        if (type !== null) {
            // First, check if default config has Origins-Pattern and comment it out
            // Sadly this is neccessary as the Origins-Patterns wont be overwritten, but merged
            script += `if [ -f "${this.defaultConfigFile}" ] && grep -q "^[[:space:]]*Unattended-Upgrade::Origins-Pattern" "${this.defaultConfigFile}"; then `;
            script += `  sed -i '/^[[:space:]]*Unattended-Upgrade::Origins-Pattern/,/^[[:space:]]*};/s/^/\\/\\/ /' "${this.defaultConfigFile}"; `;
            script += `fi; `;
