...
cockpit_web_server_add_listen_fds (CockpitWebServer *self,
                                   GError **error)
{
  gint n = sd_listen_fds (0);
  g_assert (n > 0);

  for (gint fd = SD_LISTEN_FDS_START; fd < SD_LISTEN_FDS_START + n; fd++)
    {
      GSocket *s = NULL;
      gboolean b;
      ...
      b = cockpit_web_server_add_socket (self, s, error);
      g_object_unref (s);
