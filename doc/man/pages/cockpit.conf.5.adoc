...
*StaticDir*::
  ...
  contain a `+login.html+` file and may optionally include `+login.css+`,
  `+login.js+`, `+po.js+`, and `+branding.css+`. When not set, the
  default `+/usr/share/cockpit/static+` directory is used.
