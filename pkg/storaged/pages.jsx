...
export const PageTable = ({ emptyCaption, aria_label, pages, crossrefs, sorted, show_icons }) => {
    ...
    function make_row(page, crossref, level, border, key) {
        ...
        function location_link(loc, children) {
            ...
            // HACK - somehow the link styling gets lost when putting
            // things in a Truncate. Put it back.
            const decorated_children = (
                <span style={{ textDecoration: "var(--pf-v6-c-button--TextDecorationLine)" }}>{children}</span>
            );

            return (
                <Button isInline variant="link" onClick={() => cockpit.location.go(loc)}>
                    <Truncate content={decorated_children} />
                </Button>
            );
        }
