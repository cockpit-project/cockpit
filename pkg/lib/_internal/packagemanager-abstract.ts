...
export interface MissingPackages {
    ...
    // The full package IDs corresponding to missing_names (the ID different per backend)
    missing_ids: number[]
    // Packages that were requested, are currently not installed, but can't be found in any repository
    unavailable_names: string[]
    // The to be installed package IDs, temporary kept (the ID different per backend)
    install_ids: number[]
    // The full package IDs corresponding to remove_names (the ID different per backend)
    remove_ids: number[]
...
// Enum containing PackageManager progress callback info states. These are a
// subset of the PackageKit Enum, other backends should map them to their own
// logical state.
export const Enum = {
    INFO_DOWNLOADING: 10,
    INFO_UPDATING: 11,
