    ...
    render() {
        ...
        const render = (m, term) => <CockpitNavItem
                ...
                actions={[
                    ...
                    <Button isDisabled={m === this.props.machine || m.address === "localhost"} onClick={e => this.onRemove(e, m)} className="nav-action" hidden={!editing} key={m.label + "remove"} variant="danger"><TrashIcon /></Button>
                ]}
        />;
        ...
        return (
            <div className="ct-switcher">
                <div className="pf-c-select pf-m-dark">
                    <button onClick={this.toggleMenu} id="pf-toggle-id-58" aria-labelledby=" pf-toggle-id-58" aria-expanded="false" aria-haspopup="listbox" type="button" className="pf-c-select__toggle pf-m-plain">
                        <span className="pf-c-select__toggle-wrapper">
                            <span className="pf-c-select__toggle-text desktop_v">
                        ...
                        <span className={"pf-c-select__toggle-arrow fa fa-caret-" + (this.state.opened ? "up" : "down")} aria-hidden="true" />
                    </button>
                </div>
                ...
                <HostsSelector>
                    <PageSidebar isNavOpen={this.props.opened} theme="dark" className={"sidebar-hosts" + (this.state.editing ? " edit-hosts" : "")} nav={
                        <>
                            ...
                            { this.props.machines.length > 1 &&
                                <Button isBlock variant="secondary" onClick={this.onEditHosts}>{this.state.editing ? _("Stop editing hosts") : _("Edit hosts")}</Button>
                            }
                            <Button isBlock variant="secondary" onClick={this.onAddNewHost}>{_("Add new host")}</Button>
                        </>
                    } />
                </HostsSelector>
                }
