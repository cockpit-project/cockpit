...
docker run  -d -p 4444:4444 --name selenium-hub selenium/hub:2.48.2 2> >(grep -v 'Usage of loopback devices')
wait_curl /grid/console "Grid Console"
docker run -d --link selenium-hub:hub selenium/node-chrome:2.48.2 2> >(grep -v 'Usage of loopback devices')
wait_curl /grid/console "googlechrome"
docker run -d --link selenium-hub:hub selenium/node-firefox:2.48.2 2> >(grep -v 'Usage of loopback devices')
