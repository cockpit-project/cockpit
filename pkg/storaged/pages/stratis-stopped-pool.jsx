...
import { std_reply, with_stored_passphrase } from "../stratis-utils.js";
...
export function make_stratis_stopped_pool_page(parent, uuid) {
    new_page({
        ...
        component: StoppedStratisPoolPage,
        props: { uuid },
        actions: [
            { title: _("Start"), action: () => start_pool(uuid, true), }, // XXX - show_devs?
        ],
    });
}

const StoppedStratisPoolPage = ({ page, uuid }) => {
    const key_desc = client.stratis_stopped_pool_key_description[uuid];
    const clevis_info = client.stratis_stopped_pool_clevis_info[uuid];
        ...
        <Stack hasGutter>
                ...
                <SCard title={page_type(page)} actions={<ActionButtons page={page} />}>
                    <CardBody>
                        <DescriptionList className="pf-m-horizontal-on-sm">
                            ...
                            <SDesc title={_("Keyserver")} value={ tang_url || _("none") } />
                            }
                        </DescriptionList>
                    </CardBody>
                </SCard>
            ...
            <StackItem>
                <PageCrossrefCard title={_("Block devices")}
                                  actions={null} crossrefs={get_crossrefs(uuid)} />
            </StackItem>
            <StackItem>
                <PageChildrenCard title={_("Filesystems")}
                                  emptyCaption={_("Start pool to see filesystems")}
                                  page={page} />
            </StackItem>
        </Stack>
    );
