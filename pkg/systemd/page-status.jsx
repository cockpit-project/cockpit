...
export class PageStatusNotifications extends React.Component {
    ...
    render() {
        const seen = new Map();
        return health_status.list().map(({ page, notification }) => {
            if (typeof notification?.title !== "string" || !notification.title)
                return null;

            const n = seen.get(page) ?? 0;
            seen.set(page, n + 1);
            const id_attr = "page_status_notification_" + encodeURIComponent(page).replace(/_/g, '%5F').replace(/%2F/g, '_') + (n ? `:${n}` : "");
            ...
            // 'type' is a severity or an allowlisted icon name, never a CSS class.
            const type = typeof notification.type === "string" ? notification.type : null;
            const icon = get_pficon(type) ?? icon_for_type(type);

            return (
                <li id={id_attr} key={id_attr}>
