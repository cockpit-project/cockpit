...
/* Starting a pool has to deal with a number of possibilities:
   ...
   V2: can have zero or more passphrases and clevis infos, which are
       not communicated in the StoppedPools property, just a boolean
       flag indicating whether the pool is encrypted or not.

       This will be fixed via https://github.com/stratis-storage/stratisd/pull/3811
 */
...
async function manager_start_pool(uuid, unlock_method, passphrase) {
    if (client.stratis_interface_revision < 8) {
        const start = () => client.stratis_manager.StartPool(uuid, "uuid", [true, unlock_method]).then(std_reply);
        if (passphrase) {
            const stopped_info = client.stratis_manager.StoppedPools[uuid];
            const kd = stopped_info.key_description;
            const keydesc = kd.v[1].v[1];
...
function start_pool(uuid, show_devs) {
    ...
    if (is_v1_pool(uuid)) {
        const kd = stopped_info.key_description;
        have_passphrase = kd.v[0] && kd.v[1].v[0];
        const ci = stopped_info.clevis_info;
        have_clevis = ci.v[0] && ci.v[1].v[0];
        console.log("V1", stopped_info, have_passphrase, have_clevis);
    } else {
        const encrypted = stopped_info.features.v[0] && stopped_info.features.v[1].encryption;

        // HACK - https://github.com/stratis-storage/stratisd/pull/3811
        //
        // We only know whether or not the pool is encrypted, so we
        // have to try everything.
        have_passphrase = have_clevis = encrypted;
    }
...
const StoppedStratisPoolCard = ({ card, uuid }) => {
    ...
    if (is_v1_pool(uuid)) {
        const kd = stopped_info.key_description;
        if (kd.v[0] && kd.v[1].v[0])
            key_desc = kd.v[1].v[1];
        const ci = stopped_info.clevis_info;
        if (ci.v[0] && ci.v[1].v[0])
