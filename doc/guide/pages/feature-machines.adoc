...
SSH host keys are stored in `/etc/ssh/ssh++_++known++_++hosts`.

The machine data is stored in `/etc/cockpit/machines.d/++*++.json`, or
below `$XDG++_++CONFIG++_++DIRS` if set (see
...
`"color"`::
  ...
  `rgb(r++_++value, g++_++value, b++_++value)` with each value being an
  integer between 0 and 255, or as a color name like `yellow`. When not
  given, Cockpit will assign an unused color automatically.
